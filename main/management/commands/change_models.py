    ...
    def handle(self, *args, **options):
        ...
        if model_name == 'Sran_Level':
            ...
            for music in music_list:
                ...
                print ('{0}: {1}({2}) S乱レベルID {3} --> {4}'.format(music.id, music.title, music.difficulty, music.sran_level.id, new_sran_level_id))
            ...
            for music in music_list:
                ...
                print ('{0}: {1}({2}) S乱レベルID {3} --> {4}'.format(music.id, music.title, music.difficulty, music.sran_level.id, new_sran_level.id))
