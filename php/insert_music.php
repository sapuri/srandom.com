...
function insertValues($dbh, $table_name, $max_lv, $lv, $ins_values, $exceptionMusicList) {
    ...
    foreach ($music as $element) {
        // 曲が重複していればスキップ
        if (isDuplicate($dbh, $table_name, $element[2], $element[3])) {
            continue;
        }
        ...
        echo "$music_id: ", $element[2], "(", $element[3], ") を追加しました。<br>\n";
    }
}
 ...
 */
function isDuplicate($dbh, $table_name, $title, $difficulty_id) {
    if (getMusicId($dbh, $table_name, $title, $difficulty_id)) return True;
    else return False;
}
