...
class MypageTests(TestCase):
    ...
    def test_get(self):
        resp = self.client.get(resolve_url(
            f'{APP_NAME}:mypage', username=self.user.username))
        self.assertEqual(200, resp.status_code)
...
class StatisticsTests(TestCase):
    ...
    def test_get(self):
        resp = self.client.get(resolve_url(
            f'{APP_NAME}:statistics', username=self.user.username))
        self.assertEqual(200, resp.status_code)
...
class CleardataTests(TestCase):
    ...
    def test_get(self):
        resp = self.client.get(resolve_url(
            f'{APP_NAME}:cleardata', username=self.user.username, sran_level=19))
        self.assertEqual(200, resp.status_code)
